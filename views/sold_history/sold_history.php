...
if (isset($_SESSION['user_name']) && isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'Admin'):
    ...
    // Fetch all transactions (assuming $transactions is already an array)
    // No need to call fetchAll() here as it's already an array
    ...
    $total_transactions = count($transactions); // Total number of transactions (now works because $transactions is an array)
    $total_pages = ceil($total_transactions / $items_per_page); // Total pages
    $paginated_transactions = array_slice($transactions, $offset, $items_per_page); // Slice transactions for current page
?>
